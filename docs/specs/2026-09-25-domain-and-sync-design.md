...
erDiagram
    ...
    TASK    ||--o{ TASK : "parent of (one level)"
    TASK    ||--o{ TASK_TAG : ""
    TAG     ||--o{ TASK_TAG : ""
    TASK    ||--o{ COMPLETION : "done on an occurrence"
    TASK    ||--o{ EXCEPTION : "skipped on an occurrence"

    TASK {
        uuid    id            PK "UUIDv7, generated by the client"
        string  title
        string  notes         "nullable, Markdown"
        uuid    project_id    FK "nullable"
        uuid    parent_id     FK "nullable, one level only"
        int     priority      "0..4, 0 means none"
        date    scheduled_on  "nullable"
        date    due_on        "nullable"
        string  rrule         "nullable, RFC 5545"
        date    dtstart       "nullable, anchors the rule"
        string  rank          "fractional index"
    }
    ...
    TAG {
        uuid    id            PK
        string  name          "a leading @ makes it a context"
    ...
    COMPLETION {
        uuid    id            PK
        uuid    task_id       FK
        date    occurrence    "nullable; null means a non-recurring task"
        ts      completed_at
        decimal value         "nullable, reserved for v2 habits"
    }

    EXCEPTION {
        uuid    id            PK
        uuid    task_id       FK
        date    occurrence    "a deliberately skipped date"
    }
```

**Every table above also carries four columns the diagram omits**, because they
belong to the protocol rather than to the domain and repeating them six times
would bury the model: `version` (integer), `field_ts` (JSON, one timestamp per
field), `seq` (from one sequence shared by every table) and `deleted_at`
(nullable — deletion is a tombstone). A table without them cannot take part in
synchronisation.

Three constraints carry design weight rather than being implementation detail:

- **`completion(task_id, occurrence)` is unique**, and so is
  `exception(task_id, occurrence)`. This is what makes `create` idempotent by
  natural key, on top of the operation's own id — a redelivered completion
  cannot double-apply even if its `op_id` were lost.
- **`task.parent_id` may only point at a task whose own `parent_id` is null.**
  The server enforces it; operations arrive from an offline queue and a client
  with a bug will send a three-level reference.
- **A subtask carries no `rrule`.** It lives on its parent's occurrence axis
